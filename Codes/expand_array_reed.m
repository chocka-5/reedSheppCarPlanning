function exp_array = expand_array_reed(node_x,node_y,hn,xTarget,yTarget,A,theta1,r2,ra,v,CLOSED,MAX_X,MAX_Y)
              ...
              %if (k~=1 || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                       ...
                       %e1 = hn+c;%sqrt((node_x -s_x)^2 + (node_y-s_x)^2);%exp_array(exp_count,3);%cost of travelling to node
                        %exp_array(exp_count,3) = hn+distance_reed(node_x,node_y,s_x,s_y,A,angle);
                       if((s_x == xTarget) && (s_y == yTarget))
                           exp_array(exp_count,4) = 0;
                           
                       else
                           %exp_array(exp_count,4) =abs(distance_reed((xTarget-0.5)*ra,(yTarget-0.5)*ra,(s_x-0.5)*ra,(s_y-0.5)*ra,0,theta));
                           [~,e2] = connect(reedsConnObj,[(xTarget-0.5)*ra (yTarget-0.5)*ra 0],[(s_x-0.5)*ra (s_y-0.5)*ra theta]);
                           exp_array(exp_count,4) =abs(e2);
                          %e2=abs(distance_reed((xTarget-0.5)*ra,(yTarget-0.5)*ra,(s_x-0.5)*ra,(s_y-0.5)*ra,0,theta));%distance between node and goal
                       end
                ...
                if (flag == 1)
                   ...
                   %{ 
                    ...
                    if((s_x == xTarget) && (s_y == yTarget))
                       ...
                       %exp_array(exp_count,4) =abs(distance_reed((xTarget-0.5)*ra,(yTarget-0.5)*ra,(s_x-0.5)*ra,(s_y-0.5)*ra,0,theta));
                       %e2=abs(distance_reed((xTarget-0.5)*ra,(yTarget-0.5)*ra,(s_x-0.5)*ra,(s_y-0.5)*ra,0,theta));
                       [~,e2] = connect(reedsConnObj,[(xTarget-0.5)*ra (yTarget-0.5)*ra 0],[(s_x-0.5)*ra (s_y-0.5)*ra theta]);
                       exp_array(exp_count,4) =abs(e2);
                    end
